const CompanyComplaintModel = require("./complaintCompanySchema");
...
const createComplaint = async (req, res) => {
  try {
    ...
    const newCompalaint = new CompanyComplaintModel({
      userId,
      companyId,
      complaint,
    });
...
const getAllComplaints = async (req, res) => {
  try {
    const complaints = await CompanyComplaintModel.find()
      .populate("userId")
      .populate("companyId")
      .exec();
...
const getComplaintById = async (req, res) => {
  try {
    const { id } = req.params;
    const complaint = await CompanyComplaintModel.findById(id)
      .populate("userId")
      .populate("companyId")
      .exec();
    if (!complaint) {
